...
def simulation(parameters,wind_style = 1):
        ...
        matlab = parameters[12]

        ## Parameters for wind generation and FAST simulation

        parameters_turbsim = [path_turbsim,turbsim,path_fast,path_modules,turbinefast,v,h,t,s,e,n,frequency,matlab]
        ...
        if os.path.isdir(path_fast+turbinefast+' Fundamental Load quantities - IEC61400.13 - Simulink') == False:
            os.mkdir(path_fast+turbinefast+' Fundamental Load quantities - IEC61400.13 - Simulink')
        if os.path.isdir(path_fast+turbinefast+' Fundamental Load quantities - IEC61400.13 - Simulink/'+turbinefast+' - '+str(s)+'s of duration bins')==False:
            os.mkdir(path_fast+turbinefast+' Fundamental Load quantities - IEC61400.13 - Simulink/'+turbinefast+' - '+str(s)+'s of duration bins')
        if os.path.isdir(path_fast+turbinefast+' Fundamental Load quantities - IEC61400.13 - Simulink/'+turbinefast+' - '+str(s)+'s of duration bins/'+turbinefast+'-'+str(v)+'ms')==False:
            os.mkdir(path_fast+turbinefast+' Fundamental Load quantities - IEC61400.13 - Simulink/'+turbinefast+' - '+str(s)+'s of duration bins/'+turbinefast+'-'+str(v)+'ms')

        new_path = path_fast+turbinefast+' Fundamental Load quantities - IEC61400.13 - Simulink/'+turbinefast+' - '+str(s)+'s of duration bins/'+turbinefast+'-'+str(v)+'ms/'
        ...
        insertdt[6]='         4   PCMode       - Pitch control mode {0: none, 3: user-defined from routine PitchCntrl, 4: user-defined from Simulink/Labview, 5: user-defined from Bladed-style DLL} (switch)'
        insertdt[18]='          4   VSContrl     - Variable-speed control mode {0: none, 1: simple VS, 3: user-defined from routine UserVSCont, 4: user-defined from Simulink/Labview, 5: user-defined from Bladed-style DLL} (switch)'
        ...
        eng.workspace['matlab_from_python'] = matlab
        ...
        for v in wind_speed:
                for t in turbulence_intensity:
                        for n in range(n_1,n_2+1):
                                ...
                                sim_path = path_fast+turbinefast+' Fundamental Load quantities - IEC61400.13 - Simulink/'+turbinefast+' - '+str(s)+'s of duration bins/'+turbinefast+'-'+str(v)+'ms/'
