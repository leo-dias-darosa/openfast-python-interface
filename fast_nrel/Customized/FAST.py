...
def simulation(parameters):
    ...
    if os.path.isdir(path_fast+turbinefast+' - Customized Simulation - Fast') == False:
        os.mkdir(path_fast+turbinefast+' - Customized Simulation - Fast')
    if os.path.isdir(path_fast+turbinefast+' - Customized Simulation - Fast/'+turbinefast+'-'+str(v)+'ms') == False:
        os.mkdir(path_fast+turbinefast+' - Customized Simulation - Fast/'+turbinefast+'-'+str(v)+'ms')

    new_path = path_fast+turbinefast+' - Customized Simulation - Fast/'+turbinefast+'-'+str(v)+'ms/'
    ...
    # Elasto
    Elasto_line = insert_modules[32].split()
    Beam1_line = insert_modules[33].split()
    Beam2_line = insert_modules[34].split()
    Beam3_line = insert_modules[35].split()
    Inflow_line = insert_modules[36].split()
    Aero_line = insert_modules[37].split()
    Servo_line = insert_modules[38].split()
    Hydro_line = insert_modules[39].split()
    Sub_line = insert_modules[40].split()
    Moor_line = insert_modules[41].split()
    Ice_line = insert_modules[42].split()
    ...
    airfoil_number_line=insert_aerocomponents[45].split()
    airfoil_number = int(airfoil_number_line[0])

    Aero_blade1_line = insert_aerocomponents[48+airfoil_number].split()
    Aero_blade2_line = insert_aerocomponents[49+airfoil_number].split()
    Aero_blade3_line = insert_aerocomponents[50+airfoil_number].split()
    ...
    wind_2_line=wind_inflow[16].split()
    wind_2= wind_2_line[0]
    wind_2= wind_2[1:len(wind_2)-1]

    wind_3_line=wind_inflow[20].split()
    wind_3= wind_3_line[0]
    wind_3= wind_3[1:len(wind_3)-1]

    wind_4_line=wind_inflow[22].split()
    ...
    wind_inflow[16]='"'+path_modules+path_wind+wind_name_2+'"    FileName_Uni   - Filename of time series data for uniform wind field.      (-)'
    wind_inflow[20]='"'+path_modules+path_wind+wind_name_3+'"    FileName_BTS   - Name of the Full field wind file to use (.bts)'
    wind_inflow[22]= '"'+path_modules+path_wind+wind_name_4+'"   FilenameRoot   - Rootname of the full-field wind file to use (.wnd, .sum)'
    ...
    insertmod[32]='"'+new_path+'Elasto.dat"    EDFile          - Name of file containing ElastoDyn input parameters (quoted string)'
    insertmod[33]='"'+new_path+'Beam1.dat"    BDBldFile(1)    - Name of file containing BeamDyn input parameters for blade 1 (quoted string)'
    insertmod[34]='"'+new_path+'Beam2.dat"    BDBldFile(2)    - Name of file containing BeamDyn input parameters for blade 2 (quoted string)'
    insertmod[35]='"'+new_path+'Beam3.dat"    BDBldFile(3)    - Name of file containing BeamDyn input parameters for blade 3 (quoted string)'
    insertmod[36]='"'+new_path+'Inflow.dat"    InflowFile      - Name of file containing inflow wind input parameters (quoted string)'
    insertmod[37]='"'+new_path+'Aero.dat"    AeroFile        - Name of file containing aerodynamic input parameters (quoted string)'
    insertmod[38]='"'+new_path+'Servo.dat"    ServoFile       - Name of file containing control and electrical-drive input parameters (quoted string)'
    insertmod[39]='"'+new_path+'Hydro.dat"    HydroFile       - Name of file containing hydrodynamic input parameters (quoted string)'
    insertmod[40]='"'+new_path+'Sub.dat"     SubFile         - Name of file containing sub-structural input parameters (quoted string)'
    insertmod[41]='"'+new_path+'Moor.dat"      MooringFile     - Name of file containing mooring system input parameters (quoted string)'
    insertmod[42]='"'+new_path+'Ice.dat"      IceFile         - Name of file containing ice input parameters (quoted string)'
    insertmod[47]='   %f    DT_Out          - Time step for tabular output (s) (or "default")'%(1/frequency)
    insertmod[48]='        30   TStart          - Time to begin tabular output (s)'
    open(str_fast,'w').write('\n'.join(insertmod))
         
    ## Aero_Dyn
    insert_new_aero=open(new_path+'Aero.dat').read().splitlines()
    insert_new_aero[4]='   %f   DTAero       - Time interval for aerodynamic calculations (sec)'%((1/frequency)/10)
    ...
    # Run Fast
    subprocess.run([path_fast+'openfast_x64', str_fast])
...
def data(parameters):
    ...
    sim_path = path_fast+turbinefast+' - Customized Simulation - FAST/'+turbinefast+'-'+str(v)+'ms/'
...
def plot(parameters):
    ...
    if os.path.isdir(path_fast+turbinefast+' - Customized Simulation - FAST/'+turbinefast+'-'+str(v)+'ms/'+str(t)+'ti-'+str(h)+'m-'+str(s)+'s-'+str(n)+' - Output data')==False:
        os.mkdir(path_fast+turbinefast+' - Customized Simulation - FAST/'+turbinefast+'-'+str(v)+'ms/'+str(t)+'ti-'+str(h)+'m-'+str(s)+'s-'+str(n)+' - Output data')
    ...
    output_data= sim_path+str(t)+'ti-'+str(h)+'m-'+str(s)+'s-'+str(n)+' - Output data'
    ...
    figure.set_size_inches(inches_x_time_series,inches_y_time_series)
    plt.subplot(3,1,1)
    plt.title('Tower Moments\nBase Normal',fontsize=font_size)
    ...
    HorizontalWindSpeedIndex = dataIndex.columns.get_loc("Wind1VelX")
    HorizontalWindSpeed = data_normal_col.iloc[2:,HorizontalWindSpeedIndex]
    HorizontalWindSpeed = [float(x) for x in HorizontalWindSpeed]
    ...
    plt.subplot(5,1,5)
    plt.plot(Time,HorizontalWindSpeed)
    plt.title('Horizontal Wind speed',fontsize=font_size)
    plt.ylabel('m/s',fontsize=font_size)
    plt.xlabel('Time (s)\n\nWind Speed of '+str(v)+'m/s with '+str(t)+'% turbulence intensity',fontsize=font_size)
    plt.tick_params(axis='y',labelsize = tick_size)
    plt.box(on=None)
