...
def report(parameters):
    ...
    Simulation = 'Customized FAST-Simulink'

    simulated_file = path_fast+turbinefast+' - Customized Simulation - Simulink/'+turbinefast+'-'+str(v)+'ms/'+turbinefast+'-'+str(v)+'ms-'+str(t)+'ti-'+str(h)+'m-'+str(s)+'s-'+str(n)
    
    ####################################### Get modules used #######################################

    ## Elasto
    Elasto_line = fast_input[12]
    Elasto_module_line = Elasto_line.split()
    Elasto_module = Elasto_module_line[0]
    if Elasto_module == '0':
        ElastoStatus = 'Disabled'
        BeamdynStatus = 'Disabled'
    elif Elasto_module =='1':
    ...
    Servo_module_line = Servo_line.split()
    Servo_module = Servo_module_line[0]
    ...
    Aero_module_line = Aero_line.split()
    Aero_module = Aero_module_line[0]
    if Aero_module == '0':
        AeroStatus = 'Disabled'
    elif Aero_module =='1':
    ...
    Hydro_module_line = Hydro_line.split()
    Hydro_module = Hydro_module_line[0]
    ...
    Sub_module_line = Sub_line.split()
    Sub_module = Sub_module_line[0]
    ...
    Moor_module_line = Moor_line.split()
    Moor_module = Moor_module_line[0]
    ...
    Ice_module_line = Ice_line.split()
    Ice_module = Ice_module_line[0]
    ...
    ## DOFS
    DOF = 0
    DOF_1_line=elasto_sum[8]
    DOF_1_line=DOF_1_line.split()
    DOF_1_status = DOF_1_line[0]
    if DOF_1_status == 'Enabled':
        DOF=DOF+1
        
    DOF_2_line=elasto_sum[9]
    DOF_2_line=DOF_2_line.split()
    DOF_2_status = DOF_2_line[0]
    if DOF_2_status == 'Enabled':
        DOF=DOF+1
    
    DOF_3_line=elasto_sum[10]
    DOF_3_line=DOF_3_line.split()
    DOF_3_status = DOF_3_line[0]
    if DOF_3_status == 'Enabled':
        DOF=DOF+1
    
    DOF_4_line=elasto_sum[11]
    DOF_4_line=DOF_4_line.split()
    DOF_4_status = DOF_4_line[0]
    if DOF_4_status == 'Enabled':
        DOF=DOF+1
    
    DOF_5_line=elasto_sum[12]
    DOF_5_line=DOF_5_line.split()
    DOF_5_status = DOF_5_line[0]
    if DOF_5_status == 'Enabled':
        DOF=DOF+1
    
    DOF_6_line=elasto_sum[13]
    DOF_6_line=DOF_6_line.split()
    DOF_6_status = DOF_6_line[0]
    if DOF_6_status == 'Enabled':
        DOF=DOF+1
    
    DOF_7_line=elasto_sum[14]
    DOF_7_line=DOF_7_line.split()
    DOF_7_status = DOF_7_line[0]
    if DOF_7_status == 'Enabled':
        DOF=DOF+1
    
    DOF_8_line=elasto_sum[15]
    DOF_8_line=DOF_8_line.split()
    DOF_8_status = DOF_8_line[0]
    if DOF_8_status == 'Enabled':
        DOF=DOF+1
    
    DOF_9_line=elasto_sum[16]
    DOF_9_line=DOF_9_line.split()
    DOF_9_status = DOF_9_line[0]
    if DOF_9_status == 'Enabled':
        DOF=DOF+1
    
    DOF_10_line=elasto_sum[17]
    DOF_10_line=DOF_10_line.split()
    DOF_10_status = DOF_10_line[0]
    if DOF_10_status == 'Enabled':
        DOF=DOF+1
    
    DOF_11_line=elasto_sum[18]
    DOF_11_line=DOF_11_line.split()
    DOF_11_status = DOF_11_line[0]
    if DOF_11_status == 'Enabled':
        DOF=DOF+1
    
    DOF_12_line=elasto_sum[19]
    DOF_12_line=DOF_12_line.split()
    DOF_12_status = DOF_12_line[0]
    if DOF_12_status == 'Enabled':
        DOF=DOF+1
    
    DOF_13_line=elasto_sum[20]
    DOF_13_line=DOF_13_line.split()
    DOF_13_status = DOF_13_line[0]
    if DOF_13_status == 'Enabled':
        DOF=DOF+1
    
    DOF_14_line=elasto_sum[21]
    DOF_14_line=DOF_14_line.split()
    DOF_14_status = DOF_14_line[0]
    if DOF_14_status == 'Enabled':
        DOF=DOF+1
    
    DOF_15_line=elasto_sum[22]
    DOF_15_line=DOF_15_line.split()
    DOF_15_status = DOF_15_line[0]
    if DOF_15_status == 'Enabled':
        DOF=DOF+1
    
    DOF_16_line=elasto_sum[23]
    DOF_16_line=DOF_16_line.split()
    DOF_16_status = DOF_16_line[0]
    if DOF_16_status == 'Enabled':
        DOF=DOF+1
    
    DOF_17_line=elasto_sum[24]
    DOF_17_line=DOF_17_line.split()
    DOF_17_status = DOF_17_line[0]
    if DOF_17_status == 'Enabled':
        DOF=DOF+1
    
    DOF_18_line=elasto_sum[25]
    DOF_18_line=DOF_18_line.split()
    DOF_18_status = DOF_18_line[0]
    if DOF_18_status == 'Enabled':
        DOF=DOF+1
    
    DOF_19_line=elasto_sum[26]
    DOF_19_line=DOF_19_line.split()
    DOF_19_status = DOF_19_line[0]
    if DOF_19_status == 'Enabled':
        DOF=DOF+1
    
    DOF_20_line=elasto_sum[27]
    DOF_20_line=DOF_20_line.split()
    DOF_20_status = DOF_20_line[0]
    if DOF_20_status == 'Enabled':
        DOF=DOF+1
    
    DOF_21_line=elasto_sum[28]
    DOF_21_line=DOF_21_line.split()
    DOF_21_status = DOF_21_line[0]
    if DOF_21_status == 'Enabled':
        DOF=DOF+1
    
    DOF_22_line=elasto_sum[29]
    DOF_22_line=DOF_22_line.split()
    DOF_22_status = DOF_22_line[0]
    if DOF_22_status == 'Enabled':
        DOF=DOF+1
    
    DOF_23_line=elasto_sum[30]
    DOF_23_line=DOF_23_line.split()
    DOF_23_status = DOF_23_line[0]
    if DOF_23_status == 'Enabled':
        DOF=DOF+1
    
    DOF_24_line=elasto_sum[31]
    DOF_24_line=DOF_24_line.split()
    DOF_24_status = DOF_24_line[0]
    if DOF_24_status == 'Enabled':
        DOF=DOF+1
...
#################################################################################################  AERODYNAMIC SUMMARY ################################################################################################
    ...
    aero_file = open(path_fast+turbinefast+' - Customized Simulation - Simulink/'+turbinefast+'-'+str(v)+'ms/Aero.dat').read().splitlines()
...
####################################################################################################### GET DATA #####################################################################################################
    ...
    for v in wind_speed:
        ...
        for turb in turb_list:
            if os.path.exists(path_fast+turbinefast+' - Customized Simulation - Simulink/'+turbinefast+'-'+str(v)+'ms/'+turbinefast+'-'+str(v)+'ms-'+str(turb)+'ti-'+str(h)+'m-'+str(s)+'s-'+str(n)+'.sfunc.out')==False:
                pass
            else:
                read_path = path_fast+turbinefast+' - Customized Simulation - Simulink/'+turbinefast+'-'+str(v)+'ms/'
                ...
                if np.mean(Blade1PitchAngle)<3.0 :
                    mppt_speeds.append(v)
                elif np.mean(Blade1PitchAngle)>5.0 and np.mean(Blade1PitchAngle)<80.0:
                    speed_control_speeds.append(v)
                
    ## FOR PLOTTING
    if not mppt_speeds:
        print('You need at least one MPPT region of operation simulation for generating the report')
    else:
        mppt_plot = random.choice(mppt_speeds)
    if not speed_control_speeds:
        print('You need at least one speed control region of operation simulation for generating the report')
    else:
        speed_control_plot = random.choice(speed_control_speeds)

    mppt_folder = path_fast+turbinefast+' - Customized Simulation - Simulink/'+turbinefast+'-'+str(mppt_plot)+'ms/'+str(t)+'ti-'+str(h)+'m-'+str(s)+'s-'+str(n)+' - Output data/'
    speed_control_folder = path_fast+turbinefast+' - Customized Simulation - Simulink/'+turbinefast+'-'+str(speed_control_plot)+'ms/'+str(t)+'ti-'+str(h)+'m-'+str(s)+'s-'+str(n)+' - Output data/'
...
######################################################################################################## Report ######################################################################################################
    ...
    main = r'''
        ...
        \begin{tabular}{|c|c|c|c|c|c|}
        \hline
        \textbf{Wind speed (m/s)} & \textbf{\# files} & \textbf{Min (m/$s^2$)} & \textbf{Mean (m/$s^2$)} & \textbf{Max (m/$s^2$)} & \textbf{Std dev.(m/$s^2$)} \\ \hline
        ...
        This section shows the plots of data from the tables of previous section. Two cases will be showed, one for a maximum power point tracking (MMPT) region and one for speed control region.
        ...
        \begin{figure}[H]
	    \centering
	    \includegraphics[width=0.75\linewidth]{{"'''+mppt_folder+r'''BladeRootFlapMoments"}.png}
        ...
        \begin{figure}[H]
	    \centering
	    \includegraphics[width=0.75\linewidth]{{"'''+mppt_folder+r'''BladeRootFlapMomentsFFT"}.png}
...
#######################################################################################################################################################################################################################
    ...
    tex_file= turbinefast+' FAST-Simulink - Customized Report.tex'
